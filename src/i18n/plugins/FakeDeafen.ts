/*
 * Vencord, a Discord client mod
 * Copyright (c) 2026 Vendicated and contributors
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

import { definePluginI18n } from "@utils/i18n/types";

export default definePluginI18n({
    "description": {
        "ar": "تظهر كأنك صامّ للصوت لكنك في الحقيقة لست كذلك.",
        "en": "Appear deafened to others while still hearing everything."
    },
    "options": {
        "hideIcon": {
            "ar": "إخفاء الأيقونة"
        },
        "keybind": {
            "ar": "اختصار لوحة المفاتيح"
        },
        "muteUponFakeDeafen": {
            "ar": "الكتم عند التصامّ الوهمي"
        },
        "mute": {
            "ar": "كتم"
        },
        "deafen": {
            "ar": "إصمام"
        },
        "cam": {
            "ar": "الكاميرا"
        },
        "fakeStream": {
            "ar": "بثّ وهمي: يظهر للآخرين أنك تبثّ مباشرةً دون بثّ فعلي. ⚠️ تجريبي — قد لا يظهر بثبات."
        },
        "fakeGame": {
            "ar": "نشاط وهمي: يظهر للآخرين أنك في نشاط (Watch Together) دون تشغيله لديك. ⚠️ تجريبي."
        },
        "useCustomKeybind": {
            "ar": "استخدام اختصار مخصّص"
        },
        "customKeybind": {
            "ar": "اختصار مخصّص"
        }
    }
});
