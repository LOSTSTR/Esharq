...
function showQuestEnrollmentFailureToast(quest: Quest, result: Exclude<QuestEnrollmentResult, { type: "success"; }>): void {
    const rateLimitSuffix = result.type === "rate_limited"
        ? t(" بسبب حدود المعدّل", " due to rate limits")
        : "";

    showToast(
        t(
            `فشل التسجيل في مهمة ${normalizeQuestName(quest)}${rateLimitSuffix}.`,
            `Enrollment in ${normalizeQuestName(quest)} Quest failed${rateLimitSuffix}.`
        ),
        Toasts.Type.FAILURE
    );
}
