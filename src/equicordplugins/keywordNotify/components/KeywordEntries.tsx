...
export function KeywordEntries() {
            ...
            <>
                <Collapsible title={`Keyword Entry ${i + 1}`}>
                    ...
                    <FormSwitch
                        ...
                        title={t("تجاهل البوتات", "Ignore Bots")}
                        description={t("تجاهل رسائل البوتات", "Ignore messages from bots")}
                        className={cl("ignoreCaseSwitch")} />
                    <Flex flexDirection="row">
                        <div style={{ flexGrow: 1 }}>
                            <Heading tag="h5">{t("القائمة البيضاء", "Whitelist")}</Heading>
                        </div>
                        <Button onClick={() => {
                    ...
                    <Flex flexDirection="row">
                        <div style={{ flexGrow: 1 }}>
                            <Heading tag="h5">{t("القائمة السوداء", "Blacklist")}</Heading>
                        </div>
                        <Button onClick={() => {
                    ...
                    <div className={classes(Margins.top8, Margins.bottom8)} />
                    <FormGenericLabel
                        title={t("أولوية القائمة (متقدّم)", "List priority (Advanced)")}
                        description={<>
                            <Paragraph>
    ...
    return (
        <>
            {elements}
            <div><Button onClick={() => addKeywordEntry(update)}>{t("إضافة كلمة مفتاحية", "Add Keyword Entry")}</Button></div>
        </>
    );
}
