...
function LanguageSelectionModal(props: { modalProps: RenderModalProps, src: string; }) {
    ...
    const languageOptions = [
        { label: t("كشفٌ تلقائي", "Auto Detect"), value: "auto" },
        ...
        >
            <Flex flexDirection="column" gap={20} style={{ padding: "16px" }}>
                ...
                <div>
                    ...
                    <ManaBaseRadioGroup
                        options={[{
                            name: t("نسخٌ نصّي", "Transcribe"),
                            value: "transcribe"
                        }, {
                            name: t("ترجمة إلى الإنجليزية", "Translate to English"),
                            value: "translate"
                        }]}
                        value={task}
...
function TranscriptionModal(props: { modalProps: RenderModalProps, src: string, options: { language: string, task: string; }; }) {
    ...
    return (
        <Modal
            {...modalProps}
            size="md"
            title={options.task === "translate" ? t("ترجمة رسالة صوتية", "Voice Message Translation") : t("نسخ رسالة صوتية", "Voice Message Transcription")}
            actions={[
                ...(error ? [{
                    text: t("أعد المحاولة", "Retry"),
                    variant: "primary" as const,
                    onClick: () => setRetryCount(c => c + 1)
                }] : []),
                ...(result ? [
                    {
                        text: showTimestamps ? t("إخفاء الطوابع الزمنية", "Hide Timestamps") : t("إظهار الطوابع الزمنية", "Show Timestamps"),
                        variant: "secondary" as const,
                        onClick: () => setShowTimestamps(!showTimestamps)
                    },
                    {
                        text: copied ? t("نُسخ!", "Copied!") : t("انسخ النصّ", "Copy Text"),
                        variant: "secondary" as const,
                        onClick: handleCopy
                    }
                ] : [])
            ...
            <div className={cl("content")}>
                {error ? (
                    <Flex flexDirection="column" alignItems="center" gap={12} style={{ padding: "32px 16px" }}>
                        <Heading tag="h3" style={{ color: "var(--red-360)" }}>{t("فشل النسخ النصّي", "Transcription Failed")}</Heading>
                    ...
                    <Flex flexDirection="column" alignItems="center" justifyContent="center" gap={16} style={{ height: "200px" }}>
                        <Heading tag="h3">
                            {status === "downloading_audio" && t("جارٍ تنزيل الصوت…", "Downloading Audio...")}
                            {status === "processing_audio" && t("جارٍ معالجة الصوت…", "Processing Audio...")}
                            {status === "loading" && t("جارٍ تحميل النموذج…", "Loading Model...")}
                            {status === "transcribing" && t("جارٍ النسخ النصّي…", "Transcribing...")}
                        </Heading>
                    </Flex>
                )}
            </div>
    ...
    return (
        <>
            <button
                ...
                onClick={handleClick}
                title={t("انسخ الرسالة الصوتية نصّاً", "Transcribe Voice Message")}
            >
                    ...
                    <div className={cl("embed-header")}>
                        <div className={cl("embed-title")}>
                            ...
                            <span>
                                ...
                                {status === "loading" && t("جارٍ تحميل النموذج…", "Loading Model...")}
                                {status === "transcribing" && t("جارٍ النسخ النصّي…", "Transcribing...")}
                                {status === "complete" && t("النسخ النصّي", "Transcription")}
                                {status === "error" && t("خطأ في النسخ النصّي", "Transcription Error")}
                            </span>
                        </div>
                        <div className={cl("embed-actions")}>
                            {result && (
                                <>
                                    <Tooltip text={showTimestamps ? t("إخفاء الطوابع الزمنية", "Hide Timestamps") : t("إظهار الطوابع الزمنية", "Show Timestamps")}>
                                        {props => (
                                            <button
                                                {...props}
                                    ...
                                    <Tooltip text={copied ? t("نُسخ!", "Copied!") : t("انسخ النصّ", "Copy Text")}>
                                        {props => (
                                            <button
                                                {...props}
                    ...
                    <div className={cl("embed-body")}>
                        {error ? (
                            ...
                            <Span size="xs" color="text-muted">
                                {isWorking ? t("جارٍ النسخ النصّي…", "Transcribing in progress...") : t("جارٍ التهيئة…", "Initializing...")}
                            </Span>
                        )}
                    </div>
                </div>
