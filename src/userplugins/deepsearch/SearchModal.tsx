...
function SearchResultItem({
    ...
    // نتائج الرسائل الخاصة قد يأتي مؤلّفها من الرسالة لا من المخزن، فلا يملك getAvatarURL.
        ...
        >
            ...
            <div className={cl("result-body")}>
                <div className={cl("result-header")}>
                    ...
                    {message.pinned && <span className={cl("result-pin")}>{t("مثبّتة", "Pinned")}</span>}
...
export function DeepSearchModal({ rootProps }: { rootProps: RenderModalProps; }) {
    ...
    // خارج أي سيرفر (‎@me‎) نبحث في القناة المفتوحة — وهي الطريقة الوحيدة لبحث الخاص.
    ...
    const exportResults = useCallback(async () => {
        if (results.length === 0) {
            showNotification({ title: "Deep Search", body: t("لا توجد نتائج بحث للتصدير.", "No search results to export.") });
            return;
        }
        ...
        try {
            ...
            showNotification({ title: "Deep Search", body: t("تعذّر تصدير نتائج البحث.", "Failed to export search results.") });
        }
    ...
    return (
        <ModalRoot {...rootProps} size={ModalSize.LARGE} className={cl("modal-root")}>
            <ModalHeader className={cl("header")}>
                <span className={cl("header-title")}>{t("البحث العميق", "Deep Search")}</span>
                {targetGuildId ? (
                    <span className={cl("header-guild")}>
                        {GuildStore.getGuild(targetGuildId)?.name || t("خادم", "Server")}
                    </span>
                ) : targetChannelId ? (
                    <span className={cl("header-guild")}>
                        #{ChannelStore.getChannel(targetChannelId)?.name || t("رسالة خاصة", "Direct Message")}
                    </span>
                ) : null}
                ...
                <div className={cl("container")}>
                        ...
                        {showAdvanced && (
                            <div className={cl("advanced-filters")}>
                                <div className={cl("advanced-row")}>
                                    <FilterInput
                                        label={t("من معرّف مستخدم", "From user ID")}
                                        value={filters.authorId || ""}
                                        placeholder={t("معرّف المستخدم", "User ID")}
                                        onChange={v => updateFilter("authorId", v || null)}
                                    />
                                    <FilterInput
                                        label={t("في معرّف قناة", "In channel ID")}
                                        value={filters.channelId || ""}
                                        placeholder={t("معرّف القناة", "Channel ID")}
                                        onChange={v => updateFilter("channelId", v || null)}
                                    />
                                    <FilterInput
                                        label={t("يذكر معرّف مستخدم", "Mentions user ID")}
                                ...
                                <div className={cl("advanced-row")}>
                                    <FilterInput
                                        label={t("استبعاد كلمات", "Exclude keywords")}
                                        value={filters.excludeKeywords || ""}
                                        placeholder="spoiler, leak"
                                        onChange={v => updateFilter("excludeKeywords", v || null)}
                                    />
                                    <FilterInput
                                        label={t("استبعاد نطاقات", "Exclude domains")}
                                ...
                                <div className={cl("advanced-row")}>
                                    <FilterInput
                                        label={t("بعد تاريخ", "After date")}
                                        value={filters.dateFrom || ""}
                                        placeholder="YYYY-MM-DD"
                                        onChange={v => updateFilter("dateFrom", v || null)}
                                    />
                                    <FilterInput
                                        label={t("قبل تاريخ", "Before date")}
                    ...
                    <div className={cl("results-header")}>
                        {loading ? (
                            ...
                            <span className={cl("results-stats")}>{t("لا توجد نتائج", "No results found")}</span>
                        ) : (
                            <span className={cl("results-stats")}>{t("اكتب استعلاماً أو اختر مرشّحات للبحث", "Type a query or select filters to search")}</span>
                        )}
                        {results.length > 0 && (
