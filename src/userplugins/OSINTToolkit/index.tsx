...
const OSINT_TOOLS = [
    { id: "see-know", name: "See-Know", url: "https://see-know.eu/", description: t("يبحث في الإشارات العامة على الويب.", "Searches public web signals.") },
    { id: "epieos", name: "Epieos", url: "https://epieos.com/", description: t("يفحص آثار البريد والهاتف العامة.", "Checks public email and phone traces.") },
    { id: "osintx", name: "Osintx_", url: "https://www.osintx.io/", description: t("يجمع روابط OSINT وسير العمل.", "Collects OSINT links and workflows.") },
    { id: "socialeye", name: "SocialEye", url: "https://socialeye.net/", description: t("يبحث عن أسماء المستخدمين عبر المواقع العامة.", "Searches usernames across public sites.") },
    { id: "cloudsint", name: "Cloudsint", url: "https://cloudsint.net/", description: t("يفحص انكشاف التخزين السحابي.", "Checks cloud storage exposure.") },
    { id: "proximity", name: "Proximity OSINT", url: "https://www.proximityosint.com/", description: t("يوفّر سير عمل وموارد OSINT.", "Provides OSINT workflows and resources.") },
    { id: "deadeye", name: "DeadEye", url: "https://deadeye.cc/", description: t("يبحث في إشارات الملفات الشخصية العامة.", "Searches public profile signals.") },
    { id: "indicia", name: "Indicia", url: "https://indicia.app/", description: t("يُثري المؤشّرات العامة.", "Enriches public indicators.") },
    { id: "tempemail", name: "Snapmail", url: "https://www.snapmail.in/", description: t("يُنشئ صناديق بريد مؤقتة.", "Creates temporary email inboxes.") }
] as const;

const OSINT_RESOURCES = [
    { id: "pikaosint", name: "PikaOSINT", url: "https://pikaosint.pages.dev/", description: t("مجموعة أدوات OSINT مُنتقاة.", "Curated OSINT tools collection.") },
    { id: "osintframework", name: "OSINT Framework", url: "https://osintframework.com/", description: t("فهرس موارد OSINT مُصنّف.", "Categorized OSINT resource index.") },
    { id: "photo-osint", name: "Photo OSINT", url: "https://start.me/p/0PgzqO/photo-osint", description: t("لوحة موارد للتحقيق في الصور.", "Photo investigation resource board.") }
] as const;
...
function OSINTToolkitSettingsAbout() {
    return (
        <Notice.Warning className={Margins.bottom8}>
            <p><strong>⚠️ {t(
                "هذه الأداة تفتح مواقع OSINT خارجية وتُرسل إليها معلومات الهدف العامة (اسم المستخدم، الصورة) عند النقر. استخدمها على البيانات العامة فقط واحترم خصوصية الآخرين — قد تخالف إساءة الاستخدام شروط ديسكورد والقوانين المحلية.",
                "This tool opens third-party OSINT websites and sends the target's public info (username, avatar) to them when you click. Use it on public data only and respect people's privacy — misuse may violate Discord's Terms and local laws."
            )}</strong></p>
            <p>{t("الأوامر: /domain و/iplookup و/myip و/usersearch.", "Commands: /domain, /iplookup, /myip and /usersearch.")}</p>
            <p>{t("انقر بزرّ الفأرة الأيمن على رسالة لنسخ معرّفات الكاتب وفتح عمليات بحث عن اسم المستخدم وتصفّح قوائم موارد OSINT.", "Right click a message to copy author identifiers, open username searches and browse OSINT resource lists.")}</p>
        </Notice.Warning>
    );
}
...
const messageContextMenuPatch: NavContextMenuPatchCallback = (children, { message }: MessageContextProps) => {
    ...
    children.push(
        <Menu.MenuGroup id="vc-osint-toolkit-group">
            <Menu.MenuItem id="vc-osint-toolkit" label={t("أدوات OSINT", "OSINT Toolkit")}>
                <Menu.MenuItem id="vc-osint-author" label={t("كاتب الرسالة", "Message Author")}>
                    <Menu.MenuItem
                        id="vc-osint-copy-user-id"
                        label={t("نسخ معرّف المستخدم", "Copy User ID")}
                        action={() => void copyWithToast(author.id, t("تم نسخ معرّف المستخدم.", "User ID copied."))}
                    />
                    <Menu.MenuItem
                        id="vc-osint-copy-user-url"
                        label={t("نسخ رابط المستخدم", "Copy User URL")}
                        action={() => void copyWithToast(getDiscordUserUrl(author), t("تم نسخ رابط المستخدم.", "User URL copied."))}
                    />
                    <Menu.MenuItem
                        id="vc-osint-open-user-url"
                        label={t("فتح رابط المستخدم", "Open User URL")}
                        action={() => openExternal(getDiscordUserUrl(author))}
                    />
                    <Menu.MenuItem
                        id="vc-osint-search-usersearch"
                        label={t("البحث عبر UserSearch", "Search with UserSearch")}
                        action={() => openExternal(urls.userSearch)}
                    />
                    <Menu.MenuItem
                        id="vc-osint-search-whatsmyname"
                        label={t("البحث عبر WhatsMyName", "Search with WhatsMyName")}
                    ...
                    {avatarUrl
                        ? (
                            <Menu.MenuItem
                                ...
                                label={t("بحث عكسي عن الصورة", "Reverse Search Avatar")}
                                action={() => openExternal(getAvatarSearchUrl(avatarUrl))}
                            />
                        )
                        : null}
                </Menu.MenuItem>
                <Menu.MenuItem id="vc-osint-lookup-tools" label={t("أدوات البحث", "Lookup Tools")}>
                ...
                <Menu.MenuItem id="vc-osint-resource-lists" label={t("قوائم الموارد", "Resource Lists")}>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "domain",
            description: t("يبحث عن معلومات تسجيل RDAP العامة لنطاق.", "Looks up public RDAP registration information for a domain."),
            inputType: ApplicationCommandInputType.BUILT_IN,
            options: [
                {
                    name: "domain",
                    description: t("النطاق المراد البحث عنه، مثل example.com.", "Domain to look up, like example.com."),
            ...
            execute: async (args: CommandArgument[], ctx: CommandContext) => {
                ...
                if (!isValidDomain(domain)) {
                    sendBotMessage(ctx.channel.id, { content: t("نطاق غير صالح. استخدم نطاقاً جذرياً مثل example.com.", "Invalid domain. Use a root domain like example.com.") });
                    return;
                }
                ...
                try {
                    ...
                    if (!info) {
                        sendBotMessage(ctx.channel.id, { content: t(`تعذّر جلب معلومات RDAP العامة للنطاق **${domain}**.`, `Could not retrieve public RDAP information for **${domain}**.`) });
                        return;
                    }
                    ...
                    sendBotMessage(ctx.channel.id, { content: t(`تعذّر إكمال البحث عن النطاق **${domain}**.`, `Could not complete the domain lookup for **${domain}**.`) });
                }
            }
        },
        {
            name: "iplookup",
            description: t("يبحث عن الموقع الجغرافي ومعلومات الشبكة العامة لعنوان IPv4.", "Looks up public geolocation and network information for an IPv4 address."),
            inputType: ApplicationCommandInputType.BUILT_IN,
            options: [
                {
                    name: "ip",
                    description: t("عنوان IPv4 عام للبحث عنه.", "Public IPv4 address to look up."),
                    type: ApplicationCommandOptionType.STRING,
                    required: true
                }
            ],
            execute: async (args: CommandArgument[], ctx: CommandContext) => {
                ...
                if (!isPublicIPv4(ip)) {
                    sendBotMessage(ctx.channel.id, { content: t("عنوان IPv4 عام غير صالح. استخدم عنواناً مثل 8.8.8.8.", "Invalid public IPv4 address. Use an address like 8.8.8.8.") });
                    return;
                }
                ...
                try {
                    ...
                    if (!info) {
                        sendBotMessage(ctx.channel.id, { content: t(`تعذّر جلب معلومات IP العامة للعنوان **${ip}**.`, `Could not retrieve public IP information for **${ip}**.`) });
                        return;
                    }
                    ...
                    sendBotMessage(ctx.channel.id, { content: t(`تعذّر إكمال البحث عن العنوان **${ip}**.`, `Could not complete the IP lookup for **${ip}**.`) });
                }
            }
        },
        {
            name: "myip",
            description: t("يعرض عنوان IP العام الخاص بك وموقعه التقريبي.", "Shows your public IP address and approximate geolocation."),
            inputType: ApplicationCommandInputType.BUILT_IN,
            execute: async (_args: CommandArgument[], ctx: CommandContext) => {
                try {
                    ...
                    if (!info) {
                        sendBotMessage(ctx.channel.id, { content: t("تعذّر جلب معلومات IP العام الخاص بك.", "Could not retrieve your public IP information.") });
                        return;
                    }
                    ...
                    sendBotMessage(ctx.channel.id, { content: t("تعذّر إكمال البحث عن IP العام الخاص بك.", "Could not complete your public IP lookup.") });
                }
            }
        },
        {
            name: "usersearch",
            description: t("يُنشئ روابط بحث عامة عن اسم المستخدم.", "Generates public username search links."),
            inputType: ApplicationCommandInputType.BUILT_IN,
            options: [
                {
                    name: "username",
                    description: t("اسم المستخدم المراد البحث عنه.", "Username to search."),
                    type: ApplicationCommandOptionType.STRING,
                    required: true
                }
            ],
            execute: async (args: CommandArgument[], ctx: CommandContext) => {
                ...
                if (!username) {
                    sendBotMessage(ctx.channel.id, { content: t("اسم مستخدم غير صالح.", "Invalid username.") });
                    return;
                }
