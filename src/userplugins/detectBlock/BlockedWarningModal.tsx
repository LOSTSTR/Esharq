...
const WARNING_RECIPES: Record<WarningVariant, WarningRecipe> = {
    voiceJoin: {
        title: t("الانضمام إلى المكالمة؟", "Join voice?"),
        subtitle: t("شخص حظرك موجود هنا. إن انضممت، سيبقى محظوراً.", "Someone who has blocked you is here. If you join, they will still be blocked."),
        actions: [
            { text: t("انضمام", "Join"), variant: "secondary", confirms: true },
            { text: t("عدم الانضمام", "Don't join"), variant: "primary", confirms: false },
        ],
        hearingText: t("ستتمكّنان من سماع بعضكما", "You will be able to hear each other"),
    },
    voiceLeave: {
        title: t("مغادرة المكالمة؟", "Leave voice?"),
        subtitle: t("انضمّ شخص حظرك. إن غادرت، سيبقى محظوراً.", "Someone who has blocked you has joined. If you leave, they will still be blocked."),
        actions: [
            { text: t("البقاء هنا", "Stay here"), variant: "secondary", confirms: false },
            { text: t("مغادرة", "Leave"), variant: "primary", confirms: true },
        ],
        hearingText: t("يمكنكما سماع بعضكما", "You can both hear each other"),
    },
    group: {
        title: t("مغادرة المجموعة؟", "Leave group?"),
        subtitle: t("شخص حظرك موجود في هذه المجموعة الخاصة.", "Someone who has blocked you is in this group DM."),
        actions: [
            { text: t("البقاء هنا", "Stay here"), variant: "secondary", confirms: false },
            { text: t("مغادرة", "Leave"), variant: "primary", confirms: true },
        ],
        hearingText: t("لا يزال بإمكانكما رؤية بعضكما في هذه المحادثة", "You can still see each other in this chat"),
    },
};
...
function getUserDisplayName(user: User | undefined, fallbackId: string) {
    const fallbackUser = UserStore.getUser(fallbackId);
    return user?.globalName || user?.username || fallbackUser?.globalName || fallbackUser?.username || t("مستخدم مجهول", "Unknown user");
}
...
function getBlockedSubjectText(names: string[], userIds: string[]) {
    ...
    return {
        subject,
        suffix: resolvedNames.length === 1 ? t(" موجود هنا", " is here") : t(" موجودون هنا", " are here"),
    };
}
