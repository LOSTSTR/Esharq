...
function AutoNicknameModal({ modalProps, guildId, user }: { modalProps: ModalProps; guildId: string; user: User; }) {
    ...
    return (
        <ModalRoot {...modalProps} size={ModalSize.SMALL}>
            <ModalHeader>
                <Text variant="heading-lg/semibold" tag="h1">{t("اللقب التلقائي", "Auto Nickname")}</Text>
...
const UserContextMenuPatch: NavContextMenuPatchCallback = (children, props: UserContextProps) => {
    ...
    children.push(
        <Menu.MenuItem
            id="vc-autonickname"
            label={t("اللقب التلقائي", "Auto Nickname")}
        >
            <Menu.MenuItem
            ...
            {savedTarget && (
                <Menu.MenuItem
                    ...
                    label={t("تعطيل اللقب التلقائي", "Disable Auto Nickname")}
                    color="danger"
