 ...
 * Original plugin: PasscodeLock by arg0NNY (https://github.com/okdevme/DiscordPlugins)
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 * Adapted for Esharq with four changes, each for a stated reason:
 *
 *  1. NO NETWORK. The upstream copy preloaded six decorative GIFs from imgur at module load —
 *     unconditionally, on every client start, even with the plugin disabled — which leaks the
 *     user's IP to a third party. They are replaced by an inline SVG (already present in the
 *     file) animated with CSS: no request, smaller, and smoother.
 *  2. Password hashing hardened — see the crypto section below.
 *  3. Localisation moved to Esharq's overlay system.
 *  4. The devtools recovery hatch renamed to the Esharq namespace.
 *
 * ⚠️ HONEST LIMIT: a client-side lock deters someone who walks past an unattended, unlocked
 * PC. It is NOT security against anyone with real access to the machine — they can disable the
 * plugin, edit its stored data, or open devtools. Lock the OS for that.
 */
...
// through the cracks: window.__esharqPasscodeReset()
