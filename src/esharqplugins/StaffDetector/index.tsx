...
function readFileAsDataUri(file: File): Promise<string> {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
            ...
            reject(new Error(t("تعذّرت قراءة الملف المحدَّد كبيانات صوتية.", "Selected file could not be read as audio data.")));
        };
        reader.onerror = () => reject(reader.error ?? new Error(t("تعذّرت قراءة الملف المحدَّد.", "Selected file could not be read.")));
...
function AudioUploadButton({ label, dataKey }: { label: string; dataKey: AudioDataKey; }) {
    ...
    const [filename, setFilename] = React.useState<string>(() => {
        ...
        return data ? (settings.store[nameKey] || t("تم الرفع", "Uploaded")) : "";
    });
    ...
    async function handleClick() {
        ...
        try {
            ...
            Toasts.show({
                message: t("تعذّر تحميل ذلك الملف الصوتي.", "Could not load that audio file."),
                id: Toasts.genId(),
...
const settings = definePluginSettings({
    notifHeader: {
        ...
        description: "",
        component: () => <SettingsSep title={t("الإشعارات", "Notifications")} color={C.notif} />,
    },
    showToasts: {
        type: OptionType.BOOLEAN,
        default: true,
        description: t("تنبيه منبثق داخل التطبيق عند انضمام/مغادرة أحد الطاقم.", "In-app toast alert on staff join/leave."),
    },
    showNotifications: {
        type: OptionType.BOOLEAN,
        default: false,
        description: t("إشعار سطح المكتب على مستوى النظام عند حدث الطاقم.", "OS-level desktop notification on staff event."),
    },
    notifyAlreadyInVc: {
        type: OptionType.BOOLEAN,
        default: true,
        description: t("تنبيه + تشغيل صوت الانضمام عندما يكون أحد الطاقم موجوداً مسبقاً عند دخولك القناة الصوتية.", "Alert + play join sound when staff are already present on VC join."),
    },
    enableLogs: {
        type: OptionType.BOOLEAN,
        default: false,
        description: t("طباعة أحداث StaffDetector في وحدة تحكم DevTools ‏(Ctrl+Shift+I).", "Print StaffDetector events to the DevTools console (Ctrl+Shift+I)."),
    },

    soundsHeader: {
        type: OptionType.COMPONENT,
        description: "",
        component: () => <SettingsSep title={t("الأصوات", "Sounds")} color={C.sounds} />,
    },
    enableSounds: {
        type: OptionType.BOOLEAN,
        default: true,
        description: t("تشغيل تنبيه صوتي عند انضمام/مغادرة أحد الطاقم.", "Play audio alert on staff join/leave."),
    },
    soundVolume: {
        type: OptionType.SLIDER,
        default: 0.36,
        description: t("مستوى الصوت الرئيسي لكل أصوات StaffDetector ‏(0% - 100%).", "Master volume for all StaffDetector sounds (0% - 100%)."),
    ...
    customJoinUpload: {
        type: OptionType.COMPONENT,
        description: t("ارفع صوت الانضمام (يستبدل الرفع السابق).", "Upload JOIN sound (replaces previous upload)."),
        component: () => <AudioUploadButton label={t("ارفع صوت الانضمام", "Upload JOIN Sound")} dataKey="customJoinSoundData" />,
    },
    ...
    customLeaveUpload: {
        type: OptionType.COMPONENT,
        description: t("ارفع صوت المغادرة (يستبدل الرفع السابق).", "Upload LEAVE sound (replaces previous upload)."),
        component: () => <AudioUploadButton label={t("ارفع صوت المغادرة", "Upload LEAVE Sound")} dataKey="customLeaveSoundData" />,
    },

    serverHeader: {
        type: OptionType.COMPONENT,
        description: "",
        component: () => <SettingsSep title={t("مرشّح السيرفرات", "Server Filter")} color={C.server} />,
    },
    serverFilterMode: {
        type: OptionType.SELECT,
        options: [
            { label: t("كل السيرفرات", "All servers"), value: "none", default: true },
            { label: t("تضمين السيرفرات المدرَجة فقط", "Include only listed servers"), value: "include" },
            { label: t("استثناء السيرفرات المدرَجة", "Exclude listed servers"), value: "exclude" },
        ],
        description: t("أيّ السيرفرات تُشغّل تنبيهات StaffDetector.", "Which servers trigger StaffDetector alerts."),
    },
    serverIncludeIds: {
        type: OptionType.STRING,
        default: "",
        description: t("قائمة السماح - نبّه فقط في معرّفات السيرفرات هذه. تقبل معرّفاً واحداً أو أكثر مفصولة بفاصلة أو مسافة أو شرطة.", "Allowlist - alert ONLY in these guild IDs. Accepts one or more IDs separated by comma, space, or dash."),
    },
    serverExcludeIds: {
        type: OptionType.STRING,
        default: "",
        description: t("قائمة الحظر - لا تنبّه أبداً في معرّفات السيرفرات هذه. تقبل معرّفاً واحداً أو أكثر مفصولة بفاصلة أو مسافة أو شرطة. تتجاوزها قائمة تضمين المستخدمين.", "Blocklist - never alert in these guild IDs. Accepts one or more IDs separated by comma, space, or dash. Overridden by User Include list."),
    },

    userHeader: {
        type: OptionType.COMPONENT,
        description: "",
        component: () => <SettingsSep title={t("مرشّح المستخدمين", "User Filter")} color={C.user} />,
    },
    userIncludeIds: {
        type: OptionType.STRING,
        default: "",
        description: t("تتبّع معرّفات المستخدمين هذه فقط (فارغ = كل من له صلاحيات مطابقة). يتجاوز مرشّح السيرفر وفحص الصلاحيات — يُعلَّم حتى بلا صلاحيات طاقم. مفيد لتتبّع حسابات الطاقم المتخفّية. يقبل معرّفاً واحداً أو أكثر مفصولة بفاصلة أو مسافة أو شرطة.", "Track ONLY these user IDs (empty = all with matching perms). Overrides server filter and permission check — flagged even without staff permissions. Useful to track undercover staff alts. Accepts one or more IDs separated by comma, space, or dash."),
    },
    userExcludeIds: {
        type: OptionType.STRING,
        default: "",
        description: t("تجاهل معرّفات المستخدمين هذه بغضّ النظر عن الصلاحيات. تقبل معرّفاً واحداً أو أكثر مفصولة بفاصلة أو مسافة أو شرطة.", "Ignore these user IDs regardless of permissions. Accepts one or more IDs separated by comma, space, or dash."),
    },

    permsHeader: {
        type: OptionType.COMPONENT,
        description: "",
        component: () => <SettingsSep title={t("الصلاحيات المكتشَفة", "Detected Permissions")} color={C.perms} />,
    },
...
function scanChannelStaff(channelId: string, isInit: boolean): void {
    ...
    if (isInit) {
        ...
        if (staffFound.length === 1) {
            ...
            notify("⚠️ StaffDetector:", t(`«${name}» موجود هنا مسبقاً - ${ctx}`, `"${name}" already here - ${ctx}`), getAvatarUrl(staffFound[0]));
        } else {
            ...
            notify("⚠️ StaffDetector:", t(`${staffFound.length} من الطاقم: ${names} - ${ctx}`, `${staffFound.length} staff: ${names} - ${ctx}`), getAvatarUrl(staffFound[0]));
        }
    }
}
        ...
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: Array<{ userId: string; channelId?: string; oldChannelId?: string; guildId?: string; }>; }) {
            ...
            for (let i = 0; i < voiceStates.length; i++) {
                ...
                if (entered) {
                    ...
                    playStaffSound(true);
                    notify("🚨 StaffDetector:", t(`انضمّ «${name}» - ${ctx}`, `"${name}" joined - ${ctx}`), getAvatarUrl(userId));
                    continue;
                }
                ...
                if (left && currentChannelStaff.has(userId)) {
                    ...
                    const suffix = remaining > 0 ? t(` - بقي ${remaining} من الطاقم`, ` - ${remaining} staff remaining`) : t(" - لم يبقَ أحد من الطاقم", " - No staff remaining");
                    if (settings.store.enableLogs) logger.info(`StaffDetector: "${name}" left - ${ctx} (${remaining} remaining)`);
                    playStaffSound(false);
                    notify("✅ StaffDetector:", t(`غادر «${name}» - ${ctx}${suffix}`, `"${name}" left - ${ctx}${suffix}`), getAvatarUrl(userId));
                }
            }
        },
    },
