...
async function muteAllServers() {
    ...
    Toasts.show({
        message: t("جارٍ كتم الكل ومسح الإشعارات…", "Muting all and clearing notifications…"),
    ...
    if (guildIds.length > 0) {
        ...
        for (const id of guildIds) {
            try {
                // Individual ack (safety) — RestAPI attaches auth + honours rate limits
                try { await RestAPI.post({ url: `/guilds/${id}/ack`, body: {} }); } catch { /* آمن */ }
    ...
    Toasts.show({
        message: t("تمّ كتم كل شيء ووضعه مقروءاً!", "Everything is now muted and read!"),
        ...
        children.splice(-1, 0, (
            <Menu.MenuGroup key="nc-mute-all-group">
                <Menu.MenuItem
                    ...
                    label={t("كتم كل السيرفرات ووضع علامة مقروء", "Mute all servers & mark as read")}
                    icon={() => <MuteIcon />}
