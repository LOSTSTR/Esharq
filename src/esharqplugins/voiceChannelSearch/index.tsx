            ...
            <ModalContent className="vcs-content">
                {channels === null ? (
                    <div className="vcs-loading">
                        <SpinnerIcon />
                        <span>{t("جارٍ تحميل القنوات...", "Loading channels...")}</span>
                    </div>
                ) : (
                    <>
                        <div className="vcs-search-bar">
                            ...
                            <input
                                ...
                                placeholder={t("قناة أو سيرفر...", "Channel or server...")}
                                value={query}
                        ...
                        <div className="vcs-channel-list">
                            {displayList!.length === 0 && (
                                <div className="vcs-empty">{query ? t("لا توجد قناة", "No channel found") : t("لا قنوات صوتية", "No voice channels")}</div>
                            )}
                            {displayList!.slice(0, 150).map(ch => (
                                <div key={ch.channelId}
                                    ...
                                    title={ch.canAccess ? undefined : t("لا صلاحية للانضمام إلى هذه القناة", "No permission to join this channel")}
                                >
                                    ...
                                    {joiningId === ch.channelId
                                        ? <span className="vcs-joining-label">{t("جارٍ الانضمام...", "Joining...")}</span>
                                        : ch.canAccess
                                            ? <button className="vcs-join-btn" onClick={e => { e.stopPropagation(); join(ch); }}>{t("انضمام", "Join")}</button>
                                            : <span className="vcs-locked-label">{t("خاصّة", "Private")}</span>
                                    }
                                </div>
                            ))}
                            {displayList!.length > 80 && !query && (
                                <div className="vcs-empty" style={{ fontSize: 11, opacity: 0.5 }}>
                                    {displayList!.length - 80} {t("قناة إضافية — استخدم البحث", "more channels — use search")}
                                </div>
                            )}
                        </div>
                    </>
...
function VCSHeaderButton() {
    return (
        <HeaderBarButton
            icon={SearchIcon}
            tooltip={t("بحث عن قناة صوتية", "Search voice channel")}
