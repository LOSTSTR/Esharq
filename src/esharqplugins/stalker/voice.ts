    ...
    for (const id of targets) {
        ...
        if (voiceState && (joinedVoice || switchedChannel)) {
            ...
            if (settings.store.notifyCallJoin && voiceState.channelId) {
                ...
                showNotification({
                    title: "Stalker",
                    body: t(`انضمّ ${user.username} إلى قناة صوتية: ${channelName}\nانقر للانضمام إليه.`, `${user.username} joined VC: ${channelName}\nClick to join them.`),
        ...
        if (leftVoice && lastVoiceStateForUser) {
            ...
            if (settings.store.notifyCallLeave) {
                showNotification({
                    title: "Stalker",
                    body: t(`غادر ${user.username} القناة الصوتية: ${channelName}`, `${user.username} left VC: ${channelName}`),
                    icon: user.getAvatarURL(),
                    color: NOTIFICATION_COLOR,
                });
            }
