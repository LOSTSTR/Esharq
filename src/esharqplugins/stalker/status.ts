...
const formatStatus = (status: OnlineStatus): string =>
    status === "dnd" ? t("ممنوع الإزعاج", "in dnd")
        : status === "online" ? t("متصل", "online")
            : status === "idle" ? t("خامل", "idle")
                : status === "offline" ? t("غير متصل", "offline")
                    : status;
...
export const statusChange = () => {
    ...
    for (const id of targets) {
        ...
        if (shouldNotifyForTransition(lastStatus, newStatus)) {
            showNotification({
                title: "Stalker",
                body: t(`${user.username} أصبح الآن ${formatStatus(newStatus)}`, `${user.username} is now ${formatStatus(newStatus)}`),
